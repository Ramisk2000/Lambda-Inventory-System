...
public class ProcessorUtils {
    ...
    // Schéma explicite pour les données SensorData
    public static final StructType SENSOR_DATA_SCHEMA = new StructType()
    ...
    public static void saveDataToHDFS(final JavaDStream<SensorData> dataStream, String saveFile, SparkSession sql) {
        ...
        dataStream.foreachRDD(rdd -> {
            ...
            // Créez le DataFrame avec le schéma explicite
            Dataset<Row> dataFrame = sql.createDataFrame(rdd, SensorData.class)
                    .selectExpr("id", "temperature", "humidity", "timestamp");

            dataFrame.printSchema();

            // Écrivez dans Parquet avec schéma
            dataFrame.write()
